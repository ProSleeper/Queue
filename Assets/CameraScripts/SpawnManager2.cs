...
public class SpawnManager2 : MonoBehaviour
{
    ...
    public List<GameObject> stayCube;
    public List<GameObject> sliceCube;
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0))
        {
			...
			iTween.MoveBy(mainCamera, iTween.Hash("y", 0.35f, "z", -0.35f, "time", 0.5f, "easetype", "linear"));
        }
    }
    ...
    void CubeMake()
    {
        ...
        {
            idx = 1;
            tempGameObject = stayCube[idx];
            tempGameObject.GetComponent<XMove>().isMove = true;
            tempGameObject.SetActive(true);
            tempGameObject.transform.position = new Vector3(stayCube[19].transform.position.x + Xvalue, Yvalue, stayCube[19].transform.position.z + Zvalue);
            tempGameObject.transform.localScale = stayCube[19].transform.localScale;
        }
        else
        {
            tempGameObject = stayCube[idx];
            tempGameObject.GetComponent<XMove>().isMove = true;
            tempGameObject.SetActive(true);
            tempGameObject.transform.position = new Vector3(stayCube[idx - 1].transform.position.x + Xvalue, Yvalue, stayCube[idx - 1].transform.position.z + Zvalue);
            tempGameObject.transform.localScale = stayCube[idx - 1].transform.localScale;
        }
    ...
    {
        //움직이는 블럭 멈춤
        stayCube[idx - 1].GetComponent<XMove>().isMove = false;

        Transform backOne = stayCube[idx - 2].transform;       //기준 블럭
        Transform backTwo = stayCube[idx - 1].transform;         //움직이는 블럭
        ...
        if (backTwo.GetComponent<XMove>().isLR)     //x축으로 움직일때
        {
            ...
            if (backTwo.position.x - backOne.position.x >= 0)
            {
                ...
                GameObject cut = Instantiate(sliceCube[idx - 1], new Vector3(Xvalue, backTwo.position.y, Zvalue), Quaternion.identity) as GameObject;
                cut.SetActive(true);
